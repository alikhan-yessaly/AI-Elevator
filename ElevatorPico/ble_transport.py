...
class BLETransport:
    ...
    def _irq(self, event, data):
        print("[IRQ] Event:", event, "Data:", data)  # add this — logs everything raw
        ...
        if event == _IRQ_CENTRAL_CONNECT:
            ...
            print("[BLE] GATTS write — conn:", conn_handle, "handle:", value_handle, "rx_handle:", self.rx_handle)
            if conn_handle == self.conn_handle and value_handle == self.rx_handle:
                raw = self.ble.gatts_read(self.rx_handle)
                print("[BLE] Raw write data:", raw)
            ...
            print("[BLE] GATTC notify — conn:", conn_handle, "handle:", value_handle, "data:", notify_data)
    ...
    def _handle_command(self, raw):
        ...
        if not cmd:
            ...
            return
        print("[CMD] Received raw:", raw)
        print("[CMD] Decoded cmd:", repr(cmd))
